...
df<- download.file(URL, destfile =  "./project_week4/UCI_HAR_DATASET.zip")
files<- unzip(zipfile = "./project_week4/UCI_HAR_DATASET.zip", exdir = "./project_week4", list = TRUE)

# 1. Merge train and test sets to create one data frame

#read txt file of test and training sets
subject_test<-read.table("./project_week4/UCI HAR Dataset/test/subject_test.txt")
x_test<- read.table("./project_week4/UCI HAR Dataset/test/X_test.txt")
y_test<-read.table("./project_week4/UCI HAR Dataset/test/y_test.txt")

subject_train<-read.table("./project_week4/UCI HAR Dataset/train/subject_train.txt")
x_train<- read.table("./project_week4/UCI HAR Dataset/train/X_train.txt")
y_train<- read.table("./project_week4/UCI HAR Dataset/train/y_train.txt")

#call feature vector for column names of train & test and activity labels for activity number
activity_labels<- read.table("./project_week4/UCI HAR Dataset/activity_labels.txt")
features<- read.table("./project_week4/UCI HAR Dataset/features.txt")
...
write.table(tidy_dataset, './project_week4/UCI HAR Dataset/UCIHAR_tidy_dataset.txt',row.names=FALSE,sep='\t')
